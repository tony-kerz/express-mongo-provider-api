...
import {dbgreq, isSet} from '../shared/express-helper'
...
async function getOpts(req) {
  const opts = _.transform(
    req.query,
    (result, value, key)=>{
      // npi should be a string on the document itself,
      // but in meantime convert to int here....
      if (['skip', 'limit', 'nearMiles', 'npi'].includes(key)) {
